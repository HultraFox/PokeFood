...
public class BananaTreeFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("pokefood", "banana_tree_world"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 300000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo,
					...
					BananaTreeValidPlacementProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
