...
public class LemonCollectingProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == PokefoodModBlocks.LEMON_STAGE_5.get()) {
			if (!world.isClientSide()) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x + 0.55, y + 0.55, z + 0.55), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.sweet_berry_bush.pick_berries")), SoundSource.BLOCKS, 1, 1);
					} else {
		...
		} else if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == PokefoodModBlocks.LEMON_STAGE_6.get()) {
			if (!world.isClientSide()) {
				randomNumber = Math.random();
				if (randomNumber >= 0.66) {
