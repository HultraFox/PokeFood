...
public class CookingRecipe implements Recipe<SimpleContainer> {
	...
	public static class Serializer implements RecipeSerializer<CookingRecipe> {
		...
		public static final ResourceLocation ID = new ResourceLocation("pokefood", "cooking");

		@Override
		public CookingRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
			...
			return new CookingRecipe(pRecipeId, output, inputs);
		}

		@Override
		public @Nullable CookingRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
			NonNullList<Ingredient> inputs = NonNullList.withSize(buf.readInt(), Ingredient.EMPTY);
			for (int i = 0; i < inputs.size(); i++) {
				inputs.set(i, Ingredient.fromNetwork(buf));
			}
			ItemStack output = buf.readItem();
			return new CookingRecipe(id, output, inputs);
		}
